...
export default function HeroItem({ hero, selectedHeroId }: { hero: Hero, selectedHeroId: number | null }) {
  ...
  return (
    <li onClick={() => handleSelectedHeroId(hero.id)} className={`py-1 px-2 mb-2 text-center rounded-full border-2 cursor-pointer transition-colors hover:bg-white hover:text-black ${selectedHeroId === hero.id ? "bg-banana text-black border-banana" : "bg-black text-white"}`}>
      <p>
        {hero.name}
      </p>
    </li>
